...
// -- chakra-ui custom
const MyList = chakra('li', {
  shouldForwardProp: (prop) => !["sample"].includes(prop)
})

const ChakraCreate = ({ elem, children }) => (
  chakra('li', {
    shouldForwardProp: (prop) => !["sample"].includes(prop)
  }, children)
)
...
const htmlSerializer = function (type, element, content, children, key) {
  ...
  switch (type) {
    ...
    case Elements.listItem: // 'li' - Unordered List Item
      return React.createElement('li', propsWithUniqueKey(props, key), children)

    case Elements.oListItem: // 'li' - Ordered List Item
      return React.createElement('li', propsWithUniqueKey(props, key), children) // React.createElement(MyList, propsWithUniqueKey(props, key), children)

    case Elements.list: // 'ul - Unordered List
      return React.createElement('ul', propsWithUniqueKey(props, key), children)

    case Elements.oList: // 'ol' - Ordered List
      return React.createElement('ol', propsWithUniqueKey(props, key), children) //React.createElement(ChakraCreate(OrderedList), propsWithUniqueKey(props, key), children) <OrderedList spacing={4} my={4} props={propsWithUniqueKey(props, key)}>{children}</OrderedList>
